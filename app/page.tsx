...
type OpenLot = { id:string; symbol:string; side:string; quantity:number; openDate:string; openPrice:number; cost:number; value:number; account?:string };
type ClosedTrade = { id:string; date:string; symbol:string; side:string; volume:number; result:number; account?:string; category?:string; openDate?:string; openPrice?:number; closePrice?:number; purchaseValue?:number; saleValue?:number };
...
function parseRows(rows:unknown[][],sheetName:string,target:PortfolioData,account="PLN"){
  ...
  const idx={symbol:pick(headers,["symbol","ticker"]),name:pick(headers,["instrument","instrument/position","instrument finansowy"]),category:pick(headers,["category","kategoria","product"]),type:pick(headers,["type","typ"]),time:pick(headers,["time","czas","open time","open time (utc)","czas otwarcia"]),closeTime:pick(headers,["close time","close time (utc)","czas zamkniecia"]),comment:pick(headers,["comment","komentarz"]),amount:pick(headers,["amount","kwota"]),volume:pick(headers,["volume","wolumen"]),purchase:pick(headers,["purchase value","wartosc zakupu"]),sale:pick(headers,["sale value","wartosc sprzedazy"]),value:pick(headers,["value","market value","wartosc rynkowa"]),marketPrice:pick(headers,["current price","market price","cena rynkowa"]),openPrice:pick(headers,["open price","cena otwarcia"]),closePrice:pick(headers,["close price","cena zamkniecia"]),netProfit:pick(headers,["net profit","profit/loss","zysk/strata"]),gross:pick(headers,["gross p/l","gross profit","wynik brutto"]),commission:pick(headers,["commission","open commission","prowizja"]),swap:pick(headers,["swap"])};
...
export default function Home(){
  ...
  const [view,setView]=useState<AppView>("pulpit");
  ...
  const selectView=(next:AppView)=>{setView(next);setMobileMoreOpen(false);window.scrollTo({top:0,left:0,behavior:"auto"})};
