...
import { buildDividendForecast } from "../lib/dividend-forecast";
import { calculateOpenResult, hasKnownCost } from "../lib/portfolio-metrics";

type Sector = "Technologia" | "Finanse" | "Zdrowie" | "Konsumpcja" | "Przemysł" | "Energia" | "Nieruchomości" | "ETF" | "Inne";
type Position = { id:string; symbol:string; name:string; sector:Sector; assetClass:string; quantity:number; cost:number; costKnown?:boolean; value:number; currency:string; account?:string; manual?:boolean; priceId?:string; marketPrice?:number; priceUpdatedAt?:string; priceProvider?:string; priceChangePct?:number|null };
...
type CryptoTransaction = CryptoTaxTransaction & { id:string; symbol:string; name:string; quantity:number; toSymbol?:string; toName?:string; toQuantity?:number; amount:number; currency:string; nbpRate:number; nbpDate?:string; fee:number; account:string; note?:string };
...
function CryptoPicker({label,selected,onSelect,autoFocus=false}:{label:string;selected:InstrumentResult|null;onSelect:(item:InstrumentResult|null)=>void;autoFocus?:boolean}){
  const [query,setQuery]=useState("");
  const [results,setResults]=useState<InstrumentResult[]>([]);
  const [loading,setLoading]=useState(false);
  const listId=`crypto-results-${label.normalize("NFD").replace(/[^a-zA-Z]/g,"").toLowerCase()}`;
  useEffect(()=>{if(selected||query.trim().length<2){const reset=window.setTimeout(()=>{setResults([]);setLoading(false)},0);return()=>window.clearTimeout(reset)}const controller=new AbortController();const timer=window.setTimeout(()=>{setLoading(true);fetch(`/api/instruments?q=${encodeURIComponent(query.trim())}&kind=crypto`,{signal:controller.signal}).then(async response=>{if(!response.ok)throw new Error();return response.json()}).then(result=>setResults((result.results||[]).slice(0,8))).catch(()=>{if(!controller.signal.aborted)setResults([])}).finally(()=>{if(!controller.signal.aborted)setLoading(false)})},250);return()=>{window.clearTimeout(timer);controller.abort()}},[query,selected]);
  if(selected)return <label className="field full-field"><span>{label}</span><div className="selected-instrument crypto-selected"><span className="result-mark">{selected.symbol.slice(0,2)}</span><div><strong>{selected.name}</strong><small>{selected.symbol} · CoinGecko</small></div><button type="button" onClick={()=>{onSelect(null);setQuery("")}}>Zmień</button></div></label>;
...
function authMessage(error:unknown) {
  ...
  if(code.includes("popup-closed-by-user"))return "Okno logowania Google zostało zamknięte.";
  if(code.includes("popup-blocked"))return "Przeglądarka zablokowała okno Google. Zezwól na wyskakujące okna i spróbuj ponownie.";
  if(code.includes("unauthorized-domain"))return "Ten adres aplikacji nie jest jeszcze dodany do autoryzowanych domen Firebase.";
  if(code.includes("operation-not-allowed"))return "Ten sposób logowania nie jest jeszcze włączony w Firebase.";
...
function AuthScreen({configured}:{configured:boolean}) {
  ...
  const submitGoogle=async()=>{
    ...
    try{await loginWithGoogle(remember)}catch(caught){setError(authMessage(caught))}finally{setBusy(false)}
  };
...
export default function Home({initialView="pulpit"}:{initialView?:AppView}={}){
  ...
  const [editingCost,setEditingCost]=useState<Position|null>(null);
  ...
  const [cryptoTargetInstrument,setCryptoTargetInstrument]=useState<InstrumentResult|null>(null);
  const [savingCryptoTransaction,setSavingCryptoTransaction]=useState(false);
  ...
  const [taxYear,setTaxYear]=useState(today.getFullYear()-1);
  const [taxExtraCosts,setTaxExtraCosts]=useState(0);
  const [taxNbpRates,setTaxNbpRates]=useState<NbpRate[]>([]);
  const [taxNbpLoading,setTaxNbpLoading]=useState(false);
  ...
  const refreshPrices=useCallback(async(source?:PortfolioData,announce=false)=>{
    ...
    try{
      ...
      setData(current=>{const next={...current,positions:current.positions.map(position=>{const quote=quotes.get(position.id);return quote?{...position,value:position.quantity*quote.pricePln,cost:position.assetClass==="Gotówka"?position.quantity*quote.pricePln:position.cost,costKnown:position.assetClass==="Gotówka"?true:position.costKnown,marketPrice:quote.pricePln,priceUpdatedAt:quote.updatedAt,priceProvider:quote.provider,priceChangePct:quote.changePct}:position})};portfolioRef.current=next;return next});
  ...
  const totalValue=positions.reduce((s,p)=>s+p.value,0),openResult=useMemo(()=>calculateOpenResult(positions),[positions]),totalCost=openResult.cost,openProfit=openResult.profit,realized=trades.reduce((s,t)=>s+t.result,0);
  ...
  const cryptoTransactions=data.cryptoTransactions||[];
  const taxYearOptions=useMemo(()=>taxYears(data.trades,data.cash,today.getFullYear(),data.cryptoTransactions||[]),[data.cash,data.cryptoTransactions,data.trades]);
  useEffect(()=>{if(view!=="podatki")return;const requests:{currency:string;date:string}[]=[];for(const item of data.trades){const closeYear=Number(item.date.slice(0,4));if(closeYear<taxYear-5||closeYear>taxYear||isRetirementAccount(item.account))continue;const currency=inferInstrumentCurrency(item.symbol);if(currency&&currency!=="PLN"){if(item.openDate)requests.push({currency,date:item.openDate});requests.push({currency,date:item.date})}}for(const item of data.cash){if(Number(item.date.slice(0,4))!==taxYear||isRetirementAccount(item.account)||!/divident|dividend|withholding|wht|dywidend/i.test(item.type))continue;const currency=inferInstrumentCurrency(item.symbol);if(currency&&currency!=="PLN")requests.push({currency,date:item.date})}if(!requests.length){const reset=window.setTimeout(()=>{setTaxNbpRates([]);setTaxNbpMissing(0)},0);return()=>window.clearTimeout(reset)}const controller=new AbortController();const loadingTimer=window.setTimeout(()=>setTaxNbpLoading(true),0);fetch("/api/tax/nbp",{method:"POST",headers:{"content-type":"application/json"},signal:controller.signal,body:JSON.stringify({requests})}).then(async response=>{if(!response.ok)throw new Error("NBP nie odpowiedział");return response.json()}).then(result=>{setTaxNbpRates(result.rates||[]);setTaxNbpMissing((result.missing||[]).length)}).catch(()=>{if(!controller.signal.aborted){setTaxNbpRates([]);setTaxNbpMissing(requests.length)}}).finally(()=>{if(!controller.signal.aborted)setTaxNbpLoading(false)});return()=>{window.clearTimeout(loadingTimer);controller.abort()}},[data.cash,data.trades,taxYear,view]);
  const taxTradeAudit=useMemo(()=>auditTradesWithNbp(data.trades,taxNbpRates),[data.trades,taxNbpRates]);
  const taxSummaryBeforeLosses=useMemo(()=>calculateTaxSummary({year:taxYear,trades:taxTradeAudit.auditedTrades,cash:data.cash,extraCosts:taxExtraCosts}),[data.cash,taxExtraCosts,taxTradeAudit.auditedTrades,taxYear]);
  const taxLossCarryforward=useMemo(()=>calculateLossCarryforward({targetYear:taxYear,trades:taxTradeAudit.auditedTrades,currentIncome:taxSummaryBeforeLosses.trades.result,settings:data.taxLosses}),[data.taxLosses,taxSummaryBeforeLosses.trades.result,taxTradeAudit.auditedTrades,taxYear]);
  const taxSummary=useMemo(()=>calculateTaxSummary({year:taxYear,trades:taxTradeAudit.auditedTrades,cash:data.cash,eligiblePriorLoss:taxLossCarryforward.totalDeduction,extraCosts:taxExtraCosts}),[data.cash,taxExtraCosts,taxLossCarryforward.totalDeduction,taxTradeAudit.auditedTrades,taxYear]);
  const cryptoTax=useMemo(()=>calculateCryptoTax({year:taxYear,transactions:data.cryptoTransactions||[],priorCostsOverride:data.cryptoCostOverrides?.[String(taxYear)]}),[data.cryptoCostOverrides,data.cryptoTransactions,taxYear]);
  const totalTaxDue=taxSummary.totalTaxDue+cryptoTax.tax;
  const taxYearCryptoTransactions=useMemo(()=>[...(data.cryptoTransactions||[])].filter(item=>Number(item.date.slice(0,4))===taxYear).sort((a,b)=>+new Date(b.date)-+new Date(a.date)),[data.cryptoTransactions,taxYear]);
  const taxYearTrades=useMemo(()=>taxTradeAudit.auditedTrades.filter(item=>Number(item.date.slice(0,4))===taxYear&&!isRetirementAccount(item.account)).sort((a,b)=>+new Date(b.date)-+new Date(a.date)),[taxTradeAudit.auditedTrades,taxYear]);
  const taxAuditById=useMemo(()=>new Map(taxTradeAudit.audit.map(item=>[item.id,item])),[taxTradeAudit.audit]);
  const selectedTradeAudits=useMemo(()=>taxYearTrades.map(item=>taxAuditById.get(item.id||"")).filter(Boolean),[taxAuditById,taxYearTrades]);
  const verifiedNbpTrades=selectedTradeAudits.filter(item=>item?.status==="verified").length;
  const differentNbpTrades=selectedTradeAudits.filter(item=>item?.status==="difference").length;
  const unresolvedNbpTrades=selectedTradeAudits.filter(item=>item?.status==="missing"||item?.status==="unsupported").length;
  const taxDividendAudit=useMemo(()=>{const rates=new Map(taxNbpRates.map(rate=>[rateKey(rate.currency,rate.transactionDate),rate]));return data.cash.filter(item=>Number(item.date.slice(0,4))===taxYear&&!isRetirementAccount(item.account)&&/divident|dividend|dywidend/i.test(item.type)&&inferInstrumentCurrency(item.symbol)!=="PLN").map(item=>{const currency=inferInstrumentCurrency(item.symbol);const rate=currency?rates.get(rateKey(currency,item.date)):undefined;return{...item,currency,rate,originalGross:rate?item.amount/rate.rate:undefined}})},[data.cash,taxNbpRates,taxYear]);
  const hasManualCryptoWithoutHistory=data.positions.some(item=>item.manual&&item.assetClass==="Krypto")&&!cryptoTransactions.length;
  const forecast=useMemo(()=>buildDividendForecast(dividends,cash.filter(item=>/withholding|podatek.*zrodl/i.test(norm(item.type))),today),[cash,dividends]);
  ...
  async function importFile(file?:File){
    ...
    const next:PortfolioData={positions:[],cash:[],trades:[],lots:[],taxLosses:data.taxLosses,cryptoTransactions:data.cryptoTransactions,cryptoCostOverrides:data.cryptoCostOverrides,source:file.name};
  ...
  function savePositionCost(event:React.FormEvent<HTMLFormElement>){
    event.preventDefault();if(!editingCost)return;const form=new FormData(event.currentTarget);const cost=num(form.get("cost"));
    if(cost<0){setNotice("Koszt nabycia nie może być ujemny.");return}
    const next={...data,positions:data.positions.map(position=>position.id===editingCost.id?{...position,cost,costKnown:true}:position)};
    commitPortfolio(next,`Uzupełniono koszt nabycia ${editingCost.symbol}.`);setEditingCost(null);
  }
  function clearPositionCost(){if(!editingCost)return;const next={...data,positions:data.positions.map(position=>position.id===editingCost.id?{...position,cost:0,costKnown:false}:position)};commitPortfolio(next,`Usunięto koszt nabycia ${editingCost.symbol} z obliczeń wyniku.`);setEditingCost(null)}
    ...
    try{
      ...
      const added:Position[]=aiProposal.items.map((item,index)=>{
        ...
        if(item.kind==="cash"){
          ...
          return{id:clientId("ai"),symbol:currency,name:`Gotówka ${currency}`,sector:"Inne",assetClass:"Gotówka",quantity,cost:quantity*rate,costKnown:true,value:quantity*rate,currency,account:accountName,manual:true,marketPrice:rate,priceUpdatedAt:new Date().toISOString(),priceProvider:"NBP"};
        }
        ...
        return{id:clientId("ai"),symbol:(item.symbol||item.name||"INNE").toUpperCase(),name:item.name||item.symbol||"Inne aktywo",sector:assetClass==="ETF"?"ETF":"Inne",assetClass,quantity,cost,costKnown:item.totalCostPln!==null,value,currency:"PLN",account:accountName,manual:true};
      });
  ...
  async function addCryptoTransaction(event:React.FormEvent<HTMLFormElement>){
    ...
    const form=new FormData(event.currentTarget);const text=(name:string)=>String(form.get(name)||"").trim();const amount=(name:string)=>num(text(name));const date=text("date"),quantity=amount("quantity"),toQuantity=amount("toQuantity"),transactionAmount=amount("amount"),fee=amount("fee"),currency=text("currency")||"PLN",accountName=text("account")||"Krypto";
    if(!date||quantity<=0){setNotice("Podaj datę i liczbę jednostek.");return}if(cryptoTransactionType!=="swap"&&transactionAmount<=0){setNotice("Podaj wartość transakcji.");return}if(cryptoTransactionType==="swap"&&toQuantity<=0){setNotice("Podaj liczbę otrzymanych jednostek.");return}
    setSavingCryptoTransaction(true);
    try{
      let nbpRate=1,nbpDate:string|undefined;
      if(currency!=="PLN"){
        const response=await fetch("/api/tax/nbp",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({requests:[{currency,date}]})});
  ...
  function removeCryptoTransaction(transaction:CryptoTransaction){if(!window.confirm(`Usunąć transakcję ${transaction.symbol} z ${transaction.date}?`))return;commitPortfolio({...data,cryptoTransactions:(data.cryptoTransactions||[]).filter(item=>item.id!==transaction.id)},`Usunięto transakcję ${transaction.symbol}.`)}
  ...
  function addManualAsset(event:React.FormEvent<HTMLFormElement>){
    ...
    if(manualKind==="Aktywa"){
      ...
      position={id:clientId("manual"),symbol:selectedInstrument.symbol,name:selectedInstrument.name,sector:selectedInstrument.assetClass==="ETF"?"ETF":selectedInstrument.assetClass==="Krypto"?"Inne":sectorFromSearch(selectedInstrument.sector),assetClass:selectedInstrument.assetClass,quantity,cost,costKnown:text("totalCost")!=="",value:quantity*price,currency:"PLN",account:accountName,manual:true,priceId:selectedInstrument.priceId,marketPrice:price||undefined,priceUpdatedAt:price?new Date().toISOString():undefined,priceProvider:selectedInstrument.assetClass==="Krypto"?"CoinGecko":"Yahoo Finance"};
      ...
      position={id:clientId("manual"),symbol:currency,name:`Gotówka ${currency}`,sector:"Inne",assetClass:"Gotówka",quantity,cost:quantity,costKnown:true,value:quantity,currency,account:accountName,manual:true};
    }else{
      ...
      position={id:clientId("manual"),symbol,name:text("name"),sector:"Inne",assetClass:"Inne",quantity:1,cost,costKnown:text("totalCost")!=="",value,currency:"PLN",account:accountName,manual:true};
    }
  ...
  const reportChecks:{status:"OK"|"SPRAWDŹ"|"BRAK";area:string;message:string}[]=[];
  if(taxNbpLoading)reportChecks.push({status:"BRAK",area:"Kursy NBP",message:"Poczekaj na zakończenie pobierania kursów historycznych."});
  if(unresolvedNbpTrades+taxNbpMissing>0)reportChecks.push({status:"BRAK",area:"Transakcje zagraniczne",message:`${unresolvedNbpTrades+taxNbpMissing} pozycji nie ma pełnej weryfikacji kursu NBP.`});
  if(differentNbpTrades>0)reportChecks.push({status:"SPRAWDŹ",area:"Transakcje zagraniczne",message:`${differentNbpTrades} pozycji różni się od wartości przeliczonej kursem NBP.`});
  if(taxSummary.trades.incompleteValues>0)reportChecks.push({status:"BRAK",area:"Papiery wartościowe",message:`${taxSummary.trades.incompleteValues} transakcji nie ma pełnego przychodu i kosztu.`});
  if(hasManualCryptoWithoutHistory)reportChecks.push({status:"BRAK",area:"Kryptowaluty",message:"Bieżące aktywa krypto nie mają zapisanej historii podatkowej."});
  const unconfirmedLosses=taxLossCarryforward.rows.filter(row=>row.detectedLoss>0&&data.taxLosses?.[String(row.year)]?.declaredLoss===undefined).length;
  if(unconfirmedLosses)reportChecks.push({status:"SPRAWDŹ",area:"Straty z lat ubiegłych",message:`${unconfirmedLosses} wykryte straty wymagają porównania ze złożonymi zeznaniami.`});
  const previousCryptoCarry=cryptoTax.rows.length>1?cryptoTax.rows.at(-2)?.unclaimedCosts||0:0;
  if(previousCryptoCarry>0&&data.cryptoCostOverrides?.[String(taxYear)]===undefined)reportChecks.push({status:"SPRAWDŹ",area:"Koszty krypto",message:"Koszty z poprzedniego roku są wyliczone automatycznie — porównaj je z poz. 40 poprzedniego PIT-38."});
  if(taxYear===today.getFullYear())reportChecks.push({status:"SPRAWDŹ",area:"Rok podatkowy",message:"Rok jest w trakcie i raport nie obejmuje przyszłych operacji."});
  if(taxYear!==2025)reportChecks.push({status:"SPRAWDŹ",area:"Formularz",message:"Numeracja pól została zweryfikowana wyłącznie dla PIT-38(18) za 2025 r."});
  if(!reportChecks.length)reportChecks.push({status:"OK",area:"Kompletność",message:"Nie wykryto braków w danych użytych przez kalkulator."});
  const reportBlocking=reportChecks.filter(item=>item.status==="BRAK").length,reportWarnings=reportChecks.filter(item=>item.status==="SPRAWDŹ").length;
  const reportStatus=reportBlocking?"WYMAGA DANYCH":reportWarnings?"DO SPRAWDZENIA":"GOTOWY DO KONTROLI";
  const pitReportRows:(string|number)[][]=taxYear===2025?[
    ["23","Inne przychody",taxSummary.trades.revenue,"PLN"],["24","Koszty uzyskania",taxSummary.trades.costs,"PLN"],["25","Przychody razem",taxSummary.trades.revenue,"PLN"],["26","Koszty razem",taxSummary.trades.costs,"PLN"],["27","Dochód",Math.max(0,taxSummary.trades.result),"PLN"],["28","Strata",Math.max(0,-taxSummary.trades.result),"PLN"],["29","Straty z lat ubiegłych",taxSummary.trades.eligiblePriorLoss,"PLN"],["30","Podstawa opodatkowania",taxSummary.trades.taxableBase,"PLN"],["31","Stawka podatku",19,"%"],["32","Podatek od sprzedaży",taxSummary.trades.taxBeforeCredit,"PLN"],["34","Podatek należny od sprzedaży",taxSummary.trades.tax,"PLN"],["36","Przychody z walut wirtualnych",cryptoTax.revenue,"PLN"],["37","Koszty krypto poniesione w roku",cryptoTax.currentCosts,"PLN"],["38","Koszty krypto z lat ubiegłych",cryptoTax.priorCosts,"PLN"],["39","Dochód z walut wirtualnych",cryptoTax.income,"PLN"],["40","Koszty krypto do przeniesienia",cryptoTax.unclaimedCosts,"PLN"],["41","Podstawa opodatkowania krypto",cryptoTax.taxableBase,"PLN"],["42","Stawka podatku krypto",19,"%"],["43","Podatek od krypto",cryptoTax.taxBeforeRounding,"PLN"],["46","Podatek od zagranicznych dywidend",taxSummary.foreignDividends.polishTaxBeforeCredit,"PLN"],["47","Podatek zapłacony za granicą",taxSummary.foreignDividends.credit,"PLN"],["48","Dopłata od dywidend",taxSummary.foreignDividends.taxDue,"PLN"],["50","Łączny podatek do zapłaty",totalTaxDue,"PLN"],
  ]:[["—","Brak zweryfikowanej mapy pól dla tego roku",0,"PLN"]];
  const reportSheets:SpreadsheetSheet[]=[
    {name:"Podsumowanie",title:`Kapitał — zamknięcie roku ${taxYear}`,moneyColumns:[1],rows:[["Wskaźnik","Wartość","Status / uwagi"],["Rok podatkowy",taxYear,reportStatus],["Źródło danych",data.source,"Lokalny zapis użytkownika 1"],["Wygenerowano",new Date().toLocaleString("pl-PL"),"Raport roboczy"],["Łączny podatek do zapłaty",totalTaxDue,"PIT-38"],["Podatek od papierów wartościowych",taxSummary.trades.tax,"Po odliczeniu strat"],["Podatek od krypto",cryptoTax.tax,"Osobne źródło"],["Dopłata od zagranicznych dywidend",taxSummary.foreignDividends.taxDue,"Po odliczeniu podatku zagranicznego"],["Liczba braków",reportBlocking,reportBlocking?"Uzupełnij przed rozliczeniem":"Brak"],["Liczba ostrzeżeń",reportWarnings,reportWarnings?"Sprawdź dokumenty":"Brak"]]},
    {name:"Pola PIT-38",title:`Mapa pól PIT-38 za ${taxYear}`,moneyColumns:[2],rows:[["Pozycja","Opis","Wartość","Jednostka"],...pitReportRows]},
    {name:"Akcje ETF",title:`Transakcje papierów wartościowych za ${taxYear}`,moneyColumns:[3,4,5],dateColumns:[0,8,10],rows:[["Data","Instrument","Rachunek","Przychód PLN","Koszt PLN","Wynik PLN","Status NBP","Kurs zakupu","Data tabeli zakupu","Kurs sprzedaży","Data tabeli sprzedaży"],...taxYearTrades.map(item=>{const audit=taxAuditById.get(item.id||"");return[item.date,item.symbol||"",item.account||"PLN",item.saleValue||0,item.purchaseValue||0,item.result||0,audit?.status||"brak",audit?.openRate?.rate||"",audit?.openRate?.effectiveDate||"",audit?.closeRate?.rate||"",audit?.closeRate?.effectiveDate||""]})]},
    {name:"Dywidendy",title:`Zagraniczne dywidendy za ${taxYear}`,moneyColumns:[2],dateColumns:[0,6],rows:[["Data","Instrument","Brutto PLN","Waluta","Kwota w walucie","Kurs NBP","Data tabeli NBP"],...taxDividendAudit.map(item=>[item.date,item.symbol,item.amount,item.currency||"",item.originalGross||"",item.rate?.rate||"",item.rate?.effectiveDate||""])]},
    {name:"Krypto",title:`Transakcje krypto za ${taxYear}`,moneyColumns:[8,9],dateColumns:[0,11],rows:[["Data","Operacja","Aktywo","Aktywo docelowe","Ilość","Ilość docelowa","Waluta","Kwota","Wartość PLN","Prowizja PLN","Kurs NBP","Data tabeli","Portfel / giełda","Notatka"],...taxYearCryptoTransactions.map(item=>[item.date,cryptoTypeLabels[item.type],item.symbol,item.toSymbol||"",item.quantity,item.toQuantity||"",item.currency,item.amount,item.amountPln||0,item.feePln||0,item.nbpRate,item.nbpDate||"",item.account,item.note||""])]},
    {name:"Straty",title:`Rejestr strat wykorzystanych w ${taxYear}`,moneyColumns:[2,3,4,5,6,7],integerColumns:[0,1],rows:[["Rok straty","Ważna do","Wykryta strata","Strata z zeznania","Odliczono wcześniej","Limit roczny","Odliczenie teraz","Pozostanie","Tryb","Uwzględniona"],...taxLossCarryforward.rows.map(row=>[row.year,row.expiresAfter,row.detectedLoss,row.declaredLoss,row.usedBefore,row.annualLimit,row.deduction,row.remainingAfterCurrentYear,row.strategy,row.enabled?"Tak":"Nie"])]},
    {name:"Koszty krypto",title:`Koszty walut wirtualnych do ${taxYear}`,moneyColumns:[1,2,3,4,5],integerColumns:[0],rows:[["Rok","Przychód","Koszty bieżące","Koszty z poprzednich lat","Dochód","Koszty do przeniesienia"],...cryptoTax.rows.map(row=>[row.year,row.revenue,row.currentCosts,row.priorCosts,row.income,row.unclaimedCosts])]},
    {name:"Kontrola",title:`Kontrola kompletności za ${taxYear}`,rows:[["Status","Obszar","Informacja / działanie"],...reportChecks.map(item=>[item.status,item.area,item.message])]},
    {name:"Źródła",title:"Oficjalne źródła metodyki",rows:[["Zakres","Instytucja","Adres"],["Sprzedaż akcji","Ministerstwo Finansów","https://www.podatki.gov.pl/podatki-osobiste/pit/informacje-podstawowe/co-jest-opodatkowane/zbycie-akcji"],["Waluty wirtualne","Ministerstwo Finansów","https://www.podatki.gov.pl/podatki-osobiste/pit/informacje-podstawowe/co-jest-opodatkowane/zbycie-kryptowalut"],["Odliczanie strat","Ministerstwo Finansów","https://podatki.gov.pl/ulgi-i-odliczenia/odliczenie-straty-pit"],["Broszura PIT-38 za 2025","Ministerstwo Finansów","https://www.podatki.gov.pl/media/11079/broszura-do-pit-38-za-2025-r.pdf"],["Formularze PIT","Ministerstwo Finansów","https://www.podatki.gov.pl/podatki-osobiste/pit/formularze"],["Kursy walut","Narodowy Bank Polski","https://api.nbp.pl/"]]},
  ];
  ...
  return <main className="app-shell">
    <aside className="sidebar">
      ...
      <nav className="primary-nav" aria-label="Główna nawigacja">
        ...
        <button className={view==="podatki"?"active":""} onClick={()=>selectView("podatki")}><span><ReceiptText size={18} strokeWidth={1.9}/></span>Podatki</button>
        <button className={`desktop-history ${view==="historia"?"active":""}`} onClick={()=>selectView("historia")}><span><History size={18} strokeWidth={1.9}/></span>Historia konta</button>
    ...
    {view==="dywidendy"&&<section className="subpage-grid"><article className="dividend-summary"><p className="eyebrow">Wpływy netto</p><h2>{money(divNet,2)}</h2><p>Łącznie od początku historii</p><div><span>Brutto <b>{money(divGross,2)}</b></span><span>Podatek u źródła <b>{money(Math.abs(divGross-divNet),2)}</b></span><span>Prognoza 12 mies. <b>{money(forecastTotal,2)}</b></span></div></article><article className="panel"><div className="panel-head"><div><p className="eyebrow">Prognoza</p><h3>Przyszłe wypłaty</h3></div><span className="count-pill">szacunek</span></div><div className="full-list">{forecast.map(item=><div key={`${item.symbol}-${item.date}`}><span className="status-dot forecast"/><div><strong>{item.symbol}</strong><small>{dateLabel(item.date)} · {item.confidence}</small></div><b>{money(item.net,2)}</b></div>)}{!forecast.length&&<p className="empty-inline">Brak wystarczającej, regularnej historii wypłat. Nie tworzymy prognozy z pojedynczej dywidendy.</p>}</div></article><article className="panel full"><div className="panel-head"><div><p className="eyebrow">Zaksięgowane</p><h3>Historia dywidend</h3></div><span className="count-pill">{dividends.length} wypłat</span></div><div className="full-list">{[...dividends].sort((a,b)=>+new Date(b.date)-+new Date(a.date)).map(item=><div key={item.id}><span className="status-dot"/><div><strong>{item.symbol||"Dywidenda"}</strong><small>{dateLabel(item.date)} · {item.comment}</small></div><b>{money(item.amount,2)}</b></div>)}</div></article></section>}
    {view==="podatki"&&<section className="tax-page">
      <div className="tax-toolbar"><div><span>Rok podatkowy</span><select value={taxYear} onChange={event=>setTaxYear(Number(event.target.value))} aria-label="Rok podatkowy">{taxYearOptions.map(year=><option key={year} value={year}>{year}{year===today.getFullYear()?" · w trakcie":""}</option>)}</select></div><p>Kwoty podatkowe zawsze pokazujemy w PLN, niezależnie od waluty widoku portfela.</p></div>
      <section className={`panel year-close-panel ${reportBlocking?"blocked":reportWarnings?"warning":"ready"}`}><div className="year-close-main"><div className="year-close-status"><span>{reportStatus}</span><strong>Zamknięcie roku {taxYear}</strong><p>Raport łączy pola PIT-38, transakcje, kursy NBP, dywidendy, krypto, straty i kontrolę kompletności.</p></div><div className="year-close-counts"><div><b>{reportBlocking}</b><span>braków</span></div><div><b>{reportWarnings}</b><span>do sprawdzenia</span></div></div></div><div className="year-close-checks">{reportChecks.slice(0,3).map((item,index)=><div key={`${item.area}-${index}`} className={item.status.toLowerCase()}><span>{item.status}</span><p><b>{item.area}</b>{item.message}</p></div>)}{reportChecks.length>3&&<small>+ {reportChecks.length-3} kolejnych pozycji w arkuszu „Kontrola”</small>}</div><div className="year-close-actions"><p>XLSX zawiera 8 uporządkowanych arkuszy. ZIP zawiera te same dane jako osobne pliki CSV.</p><span><button type="button" className="secondary-export" onClick={()=>exportTaxReport("csv")} disabled={taxNbpLoading}><Download size={15}/>CSV ZIP</button><button type="button" className="primary-export" onClick={()=>exportTaxReport("xlsx")} disabled={taxNbpLoading}><Download size={15}/>Pobierz XLSX</button></span></div></section>
      <section className="tax-hero-grid">
        <article className="tax-total-card"><span>Szacowany podatek do zapłaty</span><strong>{pln(totalTaxDue,0)}</strong><small>Papiery wartościowe + krypto + zagraniczne dywidendy</small></article>
        <article><span>Wynik ze sprzedaży</span><strong className={taxSummary.trades.result<0?"negative-text":""}>{pln(taxSummary.trades.result)}</strong><small>{taxSummary.trades.count} transakcji · bez IKE/IKZE</small></article>
        <article><span>Dywidendy zagraniczne</span><strong>{pln(taxSummary.foreignDividends.gross)}</strong><small>{taxSummary.foreignDividends.count} wypłat brutto</small></article>
        <article><span>Wyłączone IKE/IKZE</span><strong>{taxSummary.retirementAccounts.trades+taxSummary.retirementAccounts.dividends}</strong><small>operacji nieuwzględnionych w PIT-38</small></article>
      </section>
      <section className="tax-grid">
        <article className="panel tax-card"><div className="tax-card-head"><div><p className="eyebrow">PIT-38 · sprzedaż</p><h3>Papiery wartościowe</h3></div><span className="tax-rate">19%</span></div><div className="tax-rows"><div><span>Przychód ze sprzedaży</span><b>{pln(taxSummary.trades.revenue)}</b></div><div><span>Koszty uzyskania</span><b>{pln(taxSummary.trades.costs)}</b></div><div><span>Dochód / strata</span><b className={taxSummary.trades.result<0?"negative-text":""}>{pln(taxSummary.trades.result)}</b></div><div><span>Automatycznie odliczona strata</span><b>− {pln(taxSummary.trades.eligiblePriorLoss)}</b></div><div className="tax-result"><span>Podstawa opodatkowania</span><b>{pln(taxSummary.trades.taxableBase,0)}</b></div><div className="tax-result"><span>Podatek</span><b>{pln(taxSummary.trades.tax,0)}</b></div></div><div className="tax-inputs single"><label><span>Inne udokumentowane koszty maklerskie</span><input type="number" min="0" step="0.01" value={taxExtraCosts||""} placeholder="0,00" onChange={event=>setTaxExtraCosts(Math.max(0,Number(event.target.value)||0))}/></label></div></article>
        <article className="panel tax-card"><div className="tax-card-head"><div><p className="eyebrow">PIT-38 · dywidendy</p><h3>Zagraniczne wypłaty</h3></div><span className="tax-rate">19%</span></div><div className="tax-rows"><div><span>Dywidendy brutto</span><b>{pln(taxSummary.foreignDividends.gross)}</b></div><div><span>Polski podatek przed odliczeniem</span><b>{pln(taxSummary.foreignDividends.polishTaxBeforeCredit)}</b></div><div><span>Podatek pobrany za granicą</span><b>− {pln(taxSummary.foreignDividends.withholdingTax)}</b></div><div><span>Odliczenie z limitem 19%</span><b>− {pln(taxSummary.foreignDividends.credit)}</b></div><div className="tax-result"><span>Dopłata w Polsce</span><b>{pln(taxSummary.foreignDividends.taxDue,0)}</b></div></div><p className="tax-card-note">Polskie dywidendy: {pln(taxSummary.domesticDividends.gross)} · zwykle podatek pobiera płatnik, dlatego nie doliczamy ich ponownie.</p></article>
        <article className="panel tax-card crypto-tax-card"><div className="tax-card-head"><div><p className="eyebrow">PIT-38 · waluty wirtualne</p><h3>Kryptowaluty</h3></div><button type="button" className="compact-add" onClick={openCryptoTransactionModal}><Plus size={14}/>Dodaj transakcję</button></div><div className="tax-rows"><div><span>Przychód ze zbycia</span><b>{pln(cryptoTax.revenue)}</b></div><div><span>Koszty poniesione w {taxYear}</span><b>{pln(cryptoTax.currentCosts)}</b></div><div><span>Koszty z lat ubiegłych</span><b>{pln(cryptoTax.priorCosts)}</b></div><div><span>Dochód</span><b>{pln(cryptoTax.income)}</b></div><div><span>Koszty do przeniesienia</span><b>{pln(cryptoTax.unclaimedCosts)}</b></div><div className="tax-result"><span>Podatek</span><b>{pln(cryptoTax.tax,0)}</b></div></div><div className="crypto-override"><label><span>Koszty z poprzedniego PIT-38</span><input type="number" min="0" step="0.01" value={data.cryptoCostOverrides?.[String(taxYear)]??""} placeholder={number(cryptoTax.rows.length>1?cryptoTax.rows.at(-2)?.unclaimedCosts||0:0,2)} onChange={event=>updateCryptoCostOverride(taxYear,event.target.value===""?undefined:Math.max(0,Number(event.target.value)||0))}/></label>{data.cryptoCostOverrides?.[String(taxYear)]!==undefined&&<button type="button" onClick={()=>updateCryptoCostOverride(taxYear,undefined)}>Przywróć automat</button>}</div><p className="tax-card-note">Zamiana krypto na krypto nie tworzy przychodu ani kosztu podatkowego. Koszty potwierdź dokumentami z giełdy.</p></article>
        <article className="panel tax-card"><div className="tax-card-head"><div><p className="eyebrow">Poza bieżącym PIT-38</p><h3>IKE i IKZE</h3></div><ShieldCheck size={20}/></div><div className="tax-rows"><div><span>Zamknięte transakcje</span><b>{taxSummary.retirementAccounts.trades}</b></div><div><span>Wynik transakcji</span><b>{pln(taxSummary.retirementAccounts.tradeResult)}</b></div><div><span>Wypłaty dywidend</span><b>{taxSummary.retirementAccounts.dividends}</b></div><div><span>Dywidendy brutto</span><b>{pln(taxSummary.retirementAccounts.dividendGross)}</b></div></div><p className="tax-card-note">Operacje wewnątrz IKE/IKZE są pokazane informacyjnie. Kalkulator nie rozlicza wypłaty, zwrotu ani ulgi za wpłaty na IKZE.</p></article>
        <article className="panel tax-card tax-control-card"><div className="tax-card-head"><div><p className="eyebrow">Kontrola danych</p><h3>Wiarygodność wyliczenia</h3></div><Info size={20}/></div><ul><li className={taxNbpLoading?"warning":differentNbpTrades||unresolvedNbpTrades||taxNbpMissing?"warning":"ok"}><span/>{taxNbpLoading?"Pobieramy historyczne tabele A z NBP…":`${verifiedNbpTrades} transakcji zgodnych z NBP · ${differentNbpTrades} z różnicą · ${unresolvedNbpTrades+taxNbpMissing} bez pełnej weryfikacji.`}</li><li className={taxSummary.trades.incompleteValues?"warning":"ok"}><span/>{taxSummary.trades.incompleteValues?`${taxSummary.trades.incompleteValues} transakcji nie ma pełnej wartości zakupu i sprzedaży — użyliśmy wyniku netto.`:"Każda transakcja ma wartość zakupu i sprzedaży."}</li><li className={hasManualCryptoWithoutHistory?"warning":"ok"}><span/>{hasManualCryptoWithoutHistory?"Masz ręcznie dodane krypto, ale rejestr podatkowy nie zawiera jeszcze jego transakcji.":`${cryptoTransactions.length} transakcji w rejestrze krypto.`}</li><li className={taxYear===today.getFullYear()?"warning":"ok"}><span/>{taxYear===today.getFullYear()?"To rok w trakcie — wynik zmieni się po kolejnych operacjach.":"Wybrany rok jest zakończony."}</li></ul></article>
      </section>
      <section className="panel tax-loss-register"><div className="panel-head"><div><p className="eyebrow">Ostatnie pięć lat</p><h3>Rejestr strat do odliczenia</h3></div><span className="count-pill">odliczenie {pln(taxLossCarryforward.totalDeduction)}</span></div><p className="loss-register-intro">Najpierw wykorzystujemy najstarszą stratę. Wpisz kwoty z faktycznie złożonych zeznań oraz część wykorzystaną przed {taxYear}. Jeśli jednorazowy limit 5 mln zł był już użyty dla danej straty, zaznacz to — wtedy roczny limit wynosi 50% pierwotnej straty.</p><div className="table-wrap"><table><thead><tr><th>Uwzględnij</th><th>Rok straty</th><th>Wykryta w aplikacji</th><th>Strata z zeznania</th><th>Odliczono wcześniej</th><th>Tryb na {taxYear}</th><th>Odliczenie teraz</th><th>Pozostanie</th></tr></thead><tbody>{taxLossCarryforward.rows.map(row=>{const setting=data.taxLosses?.[String(row.year)]||{};return <tr key={row.year} className={row.enabled?"":"disabled-row"}><td><label className="loss-check"><input type="checkbox" checked={row.enabled} onChange={event=>updateTaxLoss(row.year,{enabled:event.target.checked})}/><span/></label></td><td><strong>{row.year}</strong><small className="tax-source-line">ważna do {row.expiresAfter}</small></td><td>{pln(row.detectedLoss)}</td><td><input className="loss-number-input" type="number" min="0" step="0.01" value={row.declaredLoss||""} placeholder="0,00" aria-label={`Strata wykazana za ${row.year}`} onChange={event=>updateTaxLoss(row.year,{declaredLoss:Math.max(0,Number(event.target.value)||0)})}/></td><td><input className="loss-number-input" type="number" min="0" step="0.01" value={row.usedBefore||""} placeholder="0,00" aria-label={`Strata wykorzystana przed ${taxYear} z roku ${row.year}`} onChange={event=>updateTaxLoss(row.year,{usedBefore:Math.max(0,Number(event.target.value)||0)})}/></td><td><label className="loss-method"><input type="checkbox" checked={Boolean(setting.oneTimeUsed)} onChange={event=>updateTaxLoss(row.year,{oneTimeUsed:event.target.checked})}/><span>Limit jednorazowy użyty</span></label><small className="tax-source-line">{row.strategy==="one-time"?"teraz: do 5 mln zł":"teraz: maks. 50%"}</small></td><td><strong>{pln(row.deduction)}</strong></td><td>{pln(row.remainingAfterCurrentYear)}</td></tr>})}</tbody></table></div><div className="loss-register-summary"><span>Dochód przed stratami <b>{pln(Math.max(0,taxSummaryBeforeLosses.trades.result))}</b></span><span>Odliczenie w {taxYear} <b>− {pln(taxLossCarryforward.totalDeduction)}</b></span><span>Dochód po odliczeniu <b>{pln(taxLossCarryforward.incomeAfterDeduction)}</b></span></div></section>
      <section className="panel crypto-cost-register"><div className="panel-head"><div><p className="eyebrow">Bez pięcioletniego limitu</p><h3>Koszty krypto między latami</h3></div><span className="count-pill">do przeniesienia {pln(cryptoTax.unclaimedCosts)}</span></div><p className="loss-register-intro">To nie jest strata podatkowa. Nadwyżka udokumentowanych kosztów nad przychodem przechodzi do rozliczenia w następnym roku.</p><div className="table-wrap"><table><thead><tr><th>Rok</th><th>Przychód</th><th>Koszty bieżące</th><th>Koszty z poprzednich lat</th><th>Dochód</th><th>Do przeniesienia</th></tr></thead><tbody>{cryptoTax.rows.map(row=><tr key={row.year} className={row.year===taxYear?"current-row":""}><td><strong>{row.year}</strong></td><td>{pln(row.revenue)}</td><td>{pln(row.currentCosts)}</td><td>{pln(row.priorCosts)}</td><td><strong>{pln(row.income)}</strong></td><td>{pln(row.unclaimedCosts)}</td></tr>)}</tbody></table></div></section>
      <section className="panel tax-transactions crypto-transactions"><div className="panel-head"><div><p className="eyebrow">Rejestr podatkowy</p><h3>Transakcje krypto za {taxYear}</h3></div><button type="button" className="compact-add" onClick={openCryptoTransactionModal}><Plus size={14}/>Dodaj transakcję</button></div>{taxYearCryptoTransactions.length?<div className="table-wrap"><table><thead><tr><th>Data</th><th>Operacja</th><th>Aktywo</th><th>Ilość</th><th>Wartość podatkowa</th><th>Kurs NBP</th><th></th></tr></thead><tbody>{taxYearCryptoTransactions.map(item=><tr key={item.id}><td>{dateLabel(item.date)}</td><td><span className={`crypto-kind ${item.type}`}>{cryptoTypeLabels[item.type]}</span></td><td><strong>{item.symbol}</strong>{item.type==="swap"&&<small className="tax-source-line">→ {item.toSymbol}</small>}</td><td>{number(item.quantity,6)}{item.type==="swap"&&<small className="tax-source-line">→ {number(item.toQuantity||0,6)}</small>}</td><td><strong>{item.type==="swap"?"Nieopodatkowana":pln(item.amountPln||0)}</strong>{Boolean(item.feePln)&&<small className="tax-source-line">prowizja {pln(item.feePln||0)}</small>}</td><td>{item.currency==="PLN"?"PLN":<><strong>{number(item.nbpRate,4)}</strong><small className="tax-source-line">{item.nbpDate}</small></>}</td><td><button type="button" className="row-delete" onClick={()=>removeCryptoTransaction(item)} aria-label={`Usuń transakcję ${item.symbol}`}><Trash2 size={14}/></button></td></tr>)}</tbody></table></div>:<div className="crypto-empty"><p>Brak transakcji krypto w tym roku.</p><button type="button" onClick={openCryptoTransactionModal}>Dodaj pierwszą transakcję</button></div>}</section>
      <section className="panel pit-fields"><div className="panel-head"><div><p className="eyebrow">Formularz Ministerstwa Finansów</p><h3>Mapa pól PIT-38</h3></div><span className="count-pill">{taxYear===2025?"PIT-38(18)":"sprawdź wersję"}</span></div>{taxYear===2025?<><p className="pit-fields-intro">Wariant dla kwot z raportu, których nie wykazano w PIT-8C. Jeśli otrzymasz PIT-8C, jego przychód i koszty wpisuje się odpowiednio w poz. 20 i 21, a poniższe wartości trzeba z nim uzgodnić.</p><div className="pit-fields-grid">{[
        ["23","Inne przychody",taxSummary.trades.revenue,2],["24","Koszty uzyskania",taxSummary.trades.costs,2],["25","Przychody razem",taxSummary.trades.revenue,2],["26","Koszty razem",taxSummary.trades.costs,2],["27","Dochód",Math.max(0,taxSummary.trades.result),2],["28","Strata",Math.max(0,-taxSummary.trades.result),2],["29","Straty z lat ubiegłych",taxSummary.trades.eligiblePriorLoss,2],["30","Podstawa opodatkowania",taxSummary.trades.taxableBase,0],["31","Stawka podatku",19,"percent"],["32","Podatek od sprzedaży",taxSummary.trades.taxBeforeCredit,2],["34","Podatek należny od sprzedaży",taxSummary.trades.tax,0],["36","Przychody z walut wirtualnych",cryptoTax.revenue,2],["37","Koszty krypto poniesione w roku",cryptoTax.currentCosts,2],["38","Koszty krypto z lat ubiegłych",cryptoTax.priorCosts,2],["39","Dochód z walut wirtualnych",cryptoTax.income,2],["40","Koszty krypto do przeniesienia",cryptoTax.unclaimedCosts,2],["41","Podstawa opodatkowania krypto",cryptoTax.taxableBase,0],["42","Stawka podatku krypto",19,"percent"],["43","Podatek od krypto",cryptoTax.taxBeforeRounding,2],["46","Podatek od zagranicznych dywidend",taxSummary.foreignDividends.polishTaxBeforeCredit,2],["47","Podatek zapłacony za granicą",taxSummary.foreignDividends.credit,2],["48","Dopłata od dywidend",taxSummary.foreignDividends.taxDue,0],["50","Łączny podatek do zapłaty",totalTaxDue,0],
      ].map(([position,label,value,digits])=><div key={String(position)}><span>Poz. {position}</span><p>{label}</p><strong>{digits==="percent"?`${value}%`:pln(Number(value),Number(digits))}</strong></div>)}</div></>:<p className="pit-fields-intro warning-copy">Numeracja pól formularza zmieniała się między latami. Dokładną mapę pokazujemy obecnie wyłącznie dla rozliczenia 2025 na oficjalnym formularzu PIT-38(18).</p>}</section>
      <section className="panel tax-transactions"><div className="panel-head"><div><p className="eyebrow">Audyt NBP</p><h3>Transakcje uwzględnione za {taxYear}</h3></div><span className="count-pill">{taxYearTrades.length}</span></div>{taxYearTrades.length?<div className="table-wrap"><table><thead><tr><th>Data</th><th>Instrument</th><th>Weryfikacja</th><th>Przychód</th><th>Koszt</th><th>Wynik</th></tr></thead><tbody>{taxYearTrades.map(item=>{const audit=taxAuditById.get(item.id||"");const label=audit?.status==="verified"?"Zgodne z NBP":audit?.status==="difference"?"Różnica":audit?.status==="pln"?"Transakcja PLN":audit?.status==="unsupported"?"Nieznana waluta":"Brak kursu";return <tr key={item.id}><td>{dateLabel(item.date)}</td><td><strong>{item.symbol}</strong><small className="tax-source-line">{item.account||"PLN"}</small></td><td><span className={`audit-status ${audit?.status||"missing"}`}>{label}</span></td><td><strong>{item.saleValue==null?"—":pln(item.saleValue)}</strong>{audit?.closeRate&&<small className="tax-source-line">NBP {number(audit.closeRate.rate,4)} · {audit.closeRate.effectiveDate}</small>}{audit?.nbpSale!=null&&<small className="tax-source-line">XTB {pln(audit.importedSale)} · Δ {pln(audit.saleDifference||0)}</small>}</td><td><strong>{item.purchaseValue==null?"—":pln(item.purchaseValue)}</strong>{audit?.openRate&&<small className="tax-source-line">NBP {number(audit.openRate.rate,4)} · {audit.openRate.effectiveDate}</small>}{audit?.nbpPurchase!=null&&<small className="tax-source-line">XTB {pln(audit.importedPurchase)} · Δ {pln(audit.purchaseDifference||0)}</small>}</td><td className={item.result&&item.result<0?"negative-text":"positive"}><strong>{pln(item.result||0)}</strong></td></tr>})}</tbody></table></div>:<p className="empty-inline">Brak zamkniętych transakcji na zwykłym rachunku w tym roku.</p>}</section>
      <section className="panel tax-transactions tax-dividend-audit"><div className="panel-head"><div><p className="eyebrow">Audyt NBP</p><h3>Zagraniczne dywidendy za {taxYear}</h3></div><span className="count-pill">{taxDividendAudit.length}</span></div>{taxDividendAudit.length?<div className="table-wrap"><table><thead><tr><th>Data wypłaty</th><th>Instrument</th><th>Kurs średni NBP</th><th>Data tabeli</th><th>Brutto PLN</th><th>Kwota w walucie</th></tr></thead><tbody>{taxDividendAudit.map(item=><tr key={item.id}><td>{dateLabel(item.date)}</td><td><strong>{item.symbol}</strong><small className="tax-source-line">{item.currency||"—"}</small></td><td><strong>{item.rate?number(item.rate.rate,4):"—"}</strong></td><td>{item.rate?.effectiveDate||"Brak kursu"}</td><td><strong>{pln(item.amount)}</strong></td><td>{item.originalGross==null?"—":`${number(item.originalGross,4)} ${item.currency}`}</td></tr>)}</tbody></table></div>:<p className="empty-inline">Brak zagranicznych dywidend na zwykłym rachunku w tym roku.</p>}<p className="tax-audit-foot">Kwota w walucie jest wartością kontrolną obliczoną z zaksięgowanej kwoty PLN i kursu NBP. Podstawą kalkulatora pozostaje kwota brutto zapisana w raporcie.</p></section>
      <div className="tax-disclaimer"><ReceiptText size={20}/><div><strong>To robocze wyliczenie, nie gotowa deklaracja</strong><p>Przychody i koszty walutowe sprawdzamy według średniego kursu NBP z ostatniego dnia roboczego poprzedzającego operację. Straty i koszty krypto potwierdź na faktycznie złożonych zeznaniach oraz dokumentach, a cały wynik uzgodnij z PIT-8C i kosztami spoza aplikacji.</p><div><a href="https://www.podatki.gov.pl/podatki-osobiste/pit/informacje-podstawowe/co-jest-opodatkowane/zbycie-akcji" target="_blank" rel="noreferrer">Zasady sprzedaży akcji</a><a href="https://www.podatki.gov.pl/podatki-osobiste/pit/informacje-podstawowe/co-jest-opodatkowane/zbycie-kryptowalut" target="_blank" rel="noreferrer">Zasady rozliczania krypto</a><a href="https://podatki.gov.pl/ulgi-i-odliczenia/odliczenie-straty-pit" target="_blank" rel="noreferrer">Odliczanie strat</a><a href="https://www.podatki.gov.pl/podatki-osobiste/pit/formularze" target="_blank" rel="noreferrer">Formularz i broszura PIT-38</a><a href="https://api.nbp.pl/" target="_blank" rel="noreferrer">Oficjalne API NBP</a></div></div></div>
    </section>}
    ...
    {view==="faq"&&<section className="support-page faq-page" aria-label="Najczęstsze pytania">
      <div className="faq-list">
        ...
        <details><summary>Jak dodać krypto do rozliczenia PIT-38?<span><Plus size={18}/></span></summary><p>W widoku „Podatki” wybierz „Dodaj transakcję”. Zapisz każde kupno, sprzedaż, płatność lub zamianę krypto na krypto. Aplikacja oddzieli transakcje opodatkowane od neutralnych i przeniesie niewykorzystane koszty na kolejny rok.</p></details>
        <details><summary>Skąd bierze się prognoza dywidend?<span><Plus size={18}/></span></summary><p>Prognoza wykorzystuje powtarzalny cykl rzeczywistych wypłat. Najpierw łączy wypłaty tego samego instrumentu z tego samego dnia, a częstotliwość rozpoznaje dopiero z co najmniej 2–4 dat zależnie od cyklu. Pojedyncza wypłata nie tworzy prognozy.</p></details>
    ...
    {editingCost&&<div className="modal-backdrop" role="presentation" onMouseDown={event=>{if(event.currentTarget===event.target)setEditingCost(null)}}>
      <section className="asset-modal cost-modal" role="dialog" aria-modal="true" aria-labelledby="cost-modal-title">
        ...
        <p className="cost-modal-copy">Podaj łączny udokumentowany koszt tej pozycji w PLN. Bez tej kwoty aktywo pozostaje w wartości i alokacji, ale nie wpływa na wynik ani stopę zwrotu.</p>
        <form className="asset-form" onSubmit={savePositionCost}><label className="field full-field"><span>Łączny koszt nabycia (PLN)</span><input name="cost" type="number" min="0" step="0.01" defaultValue={hasKnownCost(editingCost)?editingCost.cost:""} placeholder="np. 53000" required autoFocus/></label><div className="modal-actions cost-modal-actions">{hasKnownCost(editingCost)&&<button type="button" className="secondary-action clear-cost-action" onClick={clearPositionCost}>Oznacz jako brak danych</button>}<button type="button" className="secondary-action" onClick={()=>setEditingCost(null)}>Anuluj</button><button type="submit" className="primary-action">Zapisz koszt</button></div></form>
      </section>
    </div>}
    ...
    {addingCryptoTransaction&&<div className="modal-backdrop" role="presentation" tabIndex={-1} onKeyDown={event=>{if(event.key==="Escape")setAddingCryptoTransaction(false)}} onMouseDown={event=>{if(event.currentTarget===event.target)setAddingCryptoTransaction(false)}}>
      <section className="asset-modal crypto-transaction-modal" role="dialog" aria-modal="true" aria-labelledby="crypto-transaction-title">
        ...
        <div className="asset-type-tabs crypto-type-tabs" aria-label="Rodzaj transakcji">{(["buy","sell","swap","payment"] as CryptoTransaction["type"][]).map(kind=><button type="button" key={kind} className={cryptoTransactionType===kind?"active":""} onClick={()=>{setCryptoTransactionType(kind);if(kind!=="swap")setCryptoTargetInstrument(null)}}>{kind==="buy"?"Kupno":kind==="sell"?"Sprzedaż":kind==="swap"?"Zamiana":"Płatność"}</button>)}</div>
        <form key={cryptoTransactionType} onSubmit={event=>void addCryptoTransaction(event)}><div className="form-grid">
          <CryptoPicker label={cryptoTransactionType==="swap"?"Oddajesz kryptowalutę":"Kryptowaluta"} selected={cryptoInstrument} onSelect={setCryptoInstrument} autoFocus/>
          <label className="field"><span>Liczba jednostek</span><input name="quantity" type="number" min="0" step="any" placeholder="0,25" required/></label>
          <label className="field"><span>Data operacji</span><input name="date" type="date" max={iso(today)} defaultValue={iso(today)} required/></label>
          {cryptoTransactionType==="swap"?<><CryptoPicker label="Otrzymujesz kryptowalutę" selected={cryptoTargetInstrument} onSelect={setCryptoTargetInstrument}/><label className="field"><span>Liczba otrzymanych jednostek</span><input name="toQuantity" type="number" min="0" step="any" placeholder="1000" required/></label><div className="crypto-neutral-note field"><strong>Neutralna podatkowo</strong><p>Zamiana jednej waluty wirtualnej na inną nie tworzy przychodu ani kosztu w PIT-38.</p></div></>:<><label className="field"><span>{cryptoTransactionType==="buy"?"Zapłacona kwota":cryptoTransactionType==="sell"?"Przychód brutto":"Wartość zakupu lub zobowiązania"}</span><input name="amount" type="number" min="0" step="any" placeholder="1000" required/></label><label className="field"><span>Waluta transakcji</span><select name="currency" defaultValue="PLN"><option>PLN</option><option>EUR</option><option>USD</option><option>GBP</option></select></label><label className="field"><span>Prowizja w tej samej walucie</span><input name="fee" type="number" min="0" step="any" placeholder="0"/></label></>}
          <label className="field"><span>Portfel / giełda</span><input name="account" defaultValue="Krypto" placeholder="np. Kraken" required/></label>
          <label className="field full-field"><span>Notatka lub numer dokumentu</span><input name="note" placeholder="Opcjonalnie"/></label>
        </div><p className="form-hint">Dla USD, EUR i GBP aplikacja pobierze średni kurs NBP z ostatniego dnia roboczego przed operacją. Rejestr podatkowy nie zmienia bieżącego stanu aktywów w portfelu.</p><div className="modal-actions"><button type="button" className="secondary-action" onClick={()=>setAddingCryptoTransaction(false)}>Anuluj</button><button type="submit" className="primary-action" disabled={savingCryptoTransaction||!cryptoInstrument||(cryptoTransactionType==="swap"&&!cryptoTargetInstrument)}>{savingCryptoTransaction?"Przeliczam kurs…":"Zapisz transakcję"}</button></div></form>
      </section>
    </div>}
