...
type ManualKind = "Krypto" | "Gotówka" | "Inne";
type CashEvent = { id:string; date:string; type:string; symbol:string; comment:string; amount:number; account?:string };
type ClosedTrade = { id:string; date:string; symbol:string; side:string; volume:number; result:number; account?:string };
...
function parseRows(rows:unknown[][],sheetName:string,target:PortfolioData,account="PLN"){
  ...
  const idx={symbol:pick(headers,["symbol","ticker"]),name:pick(headers,["instrument","instrument/position","instrument finansowy"]),category:pick(headers,["category","kategoria","product"]),type:pick(headers,["type","typ"]),time:pick(headers,["time","czas","open time","open time (utc)","czas otwarcia"]),closeTime:pick(headers,["close time","close time (utc)","czas zamkniecia"]),comment:pick(headers,["comment","komentarz"]),amount:pick(headers,["amount","kwota"]),volume:pick(headers,["volume","wolumen"]),purchase:pick(headers,["purchase value","wartosc zakupu"]),value:pick(headers,["value","market value","wartosc rynkowa"]),marketPrice:pick(headers,["current price","market price","cena rynkowa"]),openPrice:pick(headers,["open price","cena otwarcia"]),netProfit:pick(headers,["net profit","profit/loss","zysk/strata"]),gross:pick(headers,["gross p/l","gross profit","wynik brutto"]),commission:pick(headers,["commission","open commission","prowizja"]),swap:pick(headers,["swap"])};
  ...
  if(isCash)dataRows.forEach((row,i)=>{const type=String(row[idx.type]??"").trim();const amount=num(row[idx.amount]);if(norm(type)==="total"||(!type&&!amount))return;target.cash.push({id:`cash-${account}-${sheetName}-${i}`,date:asDate(row[idx.time]),type,symbol:reportText(row[idx.symbol]),comment:reportText(row[idx.comment]),amount,account})});
  else if(isClosed)dataRows.forEach((row,i)=>{const symbol=String(row[idx.symbol]??"").trim();if(!symbol)return;const result=idx.netProfit>=0?num(row[idx.netProfit]):num(row[idx.gross])+num(row[idx.commission])+num(row[idx.swap]);target.trades.push({id:`trade-${account}-${sheetName}-${i}`,date:asDate(row[idx.closeTime>=0?idx.closeTime:idx.time]),symbol,side:String(row[idx.type]??"").trim(),volume:num(row[idx.volume]),result,account})});
...
function Donut({items,total}:{items:{label:string;value:number;color:string}[];total:number}){const stops=items.map((item,index)=>{const start=items.slice(0,index).reduce((sum,row)=>sum+(total?row.value/total*360:0),0);const end=start+(total?item.value/total*360:0);return`${item.color} ${start}deg ${end}deg`}).join(", ");return <div className="donut" style={{background:`conic-gradient(${stops||"#e4e6df 0 360deg"})`}}><div className="donut-hole"><span>Wartość</span><strong>{money(total)}</strong></div></div>}
...
export default function Home(){
  ...
  const [manualKind,setManualKind]=useState<ManualKind>("Krypto");
  ...
  const refreshPrices=useCallback(async(source?:PortfolioData,announce=false)=>{
    ...
    try{
      ...
      setData(current=>{const next={...current,positions:current.positions.map(position=>{const quote=quotes.get(position.id);return quote?{...position,value:position.quantity*quote.pricePln,marketPrice:quote.pricePln,priceUpdatedAt:quote.updatedAt,priceProvider:quote.provider,priceChangePct:quote.changePct}:position})};portfolioRef.current=next;return next});
  ...
  const allocationItems=useMemo(()=>{const map=new Map<string,number>();positions.forEach(p=>{const key=allocation==="firma"?p.symbol:allocation==="sektor"?p.sector:p.assetClass;map.set(key,(map.get(key)||0)+p.value)});const colors=["#2e6b4f","#e3a73a","#26342e","#8bb29e","#e4774f","#aeb5a8","#697b70","#d4c6a6"];return[...map.entries()].sort((a,b)=>b[1]-a[1]).map(([label,value],i)=>({label,value,color:colors[i%colors.length]}))},[positions,allocation]);
  async function importFile(file?:File){
    if(!file)return;setImporting(true);setNotice("");
    const next:PortfolioData={positions:[],cash:[],trades:[],source:file.name};
  ...
  function addManualAsset(event:React.FormEvent<HTMLFormElement>){
    ...
    if(manualKind==="Krypto"){
      const symbol=text("symbol").toUpperCase(),quantity=amount("quantity"),averagePrice=amount("averagePrice");
      if(!symbol||quantity<=0){setNotice("Uzupełnij symbol i ilość krypto.");return}
      position={id:`manual-${Date.now()}`,symbol,name:text("name")||symbol,sector:"Inne",assetClass:"Krypto",quantity,cost:quantity*averagePrice,value:quantity*averagePrice,currency:"PLN",account:accountName,manual:true,priceId:text("priceId").toLowerCase()||undefined};
    }else if(manualKind==="Gotówka"){
      const currency=text("currency").toUpperCase()||"PLN",quantity=amount("quantity"),fx=amount("fx")||1;
      if(quantity<=0){setNotice("Kwota gotówki musi być większa od zera.");return}
      position={id:`manual-${Date.now()}`,symbol:currency,name:`Gotówka ${currency}`,sector:"Inne",assetClass:"Gotówka",quantity,cost:quantity*fx,value:quantity*fx,currency,account:accountName,manual:true};
    }else{
    ...
    commitPortfolio(next,`Dodano ${position.name} do portfela.`);setAccount("Wszystkie");setAddingAsset(false);if(position.assetClass==="Krypto"||position.assetClass==="Gotówka")void refreshPrices(next);
  }
  ...
  return <main className="app-shell">
    <aside className="sidebar"><div className="brand"><span className="brand-mark">K</span><span>KAPITAŁ<small>prywatny portfel</small></span></div><nav aria-label="Główna nawigacja"><button className={view==="pulpit"?"active":""} onClick={()=>setView("pulpit")}><span>◫</span>Pulpit</button><button className={view==="dywidendy"?"active":""} onClick={()=>setView("dywidendy")}><span>↗</span>Dywidendy</button><button className={view==="historia"?"active":""} onClick={()=>setView("historia")}><span>≡</span>Historia konta</button></nav><div className="privacy"><span>●</span><div><strong>Raport zostaje prywatny</strong><p>Do serwisów cenowych trafiają tylko symbole instrumentów.</p></div></div><button className="text-button" onClick={()=>setData(demoData)}>Przywróć dane demo</button></aside>
    <section className="content"><header className="topbar"><div><p className="eyebrow">{data.source}</p><h1>{view==="pulpit"?"Twój portfel":view==="dywidendy"?"Dywidendy":"Historia konta"}</h1></div><div className="top-actions">{accounts.length>1&&<select className="account-select" value={account} onChange={e=>setAccount(e.target.value)} aria-label="Rachunek">{accounts.map(item=><option key={item}>{item}</option>)}</select>}<button className="add-asset-button" onClick={()=>setAddingAsset(true)}><span>＋</span>Dodaj aktywo</button><span className="as-of">Stan na dziś</span><button className="import-button" onClick={()=>fileRef.current?.click()} disabled={importing}><span>＋</span>{importing?"Wczytuję…":"Importuj XTB"}</button></div><input ref={fileRef} className="sr-only" type="file" accept=".zip,.xlsx,.xls,.csv" onChange={e=>importFile(e.target.files?.[0])}/></header>
    <div className={`price-status ${priceStatus.loading?"loading":priceStatus.missing.length?"partial":"live"}`} role="status"><span className="live-dot"/><div><strong>{priceStatus.loading?"Pobieram aktualne ceny…":priceStatus.updatedAt?"Notowania są aktualne":"Oczekiwanie na notowania"}</strong><small>{priceStatus.updatedAt?`${priceStatus.updated} instrumentów · aktualizacja ${new Intl.DateTimeFormat("pl-PL",{hour:"2-digit",minute:"2-digit"}).format(new Date(priceStatus.updatedAt))}${priceStatus.missing.length?` · ${new Set(priceStatus.missing).size} bez ceny`:""}`:"Ceny odświeżą się automatycznie"}</small></div><button type="button" onClick={()=>void refreshPrices(undefined,true)} disabled={priceStatus.loading}>↻ Odśwież</button></div>
    ...
    <section className="panel positions-panel"><div className="panel-head"><div><p className="eyebrow">Aktywne pozycje</p><h3>Aktywa</h3></div><span className="count-pill">{positions.length} pozycji</span></div>{positions.length?<div className="table-wrap"><table><thead><tr><th>Instrument</th><th>Rachunek</th><th>Sektor</th><th>Ilość</th><th>Aktualna cena</th><th>Wartość</th><th>Koszt</th><th>Wynik</th><th>Udział</th></tr></thead><tbody>{positions.map(p=>{const profit=p.value-p.cost;return <tr key={p.id}><td><div className="asset"><span>{p.symbol.slice(0,2)}</span><div><strong>{p.symbol}</strong><small>{p.name}</small></div></div></td><td><span className="account-pill">{p.account||"PLN"}</span></td><td><select value={p.sector} onChange={e=>updateSector(p.id,e.target.value as Sector)} aria-label={`Sektor ${p.symbol}`}>{sectors.map(sector=><option key={sector}>{sector}</option>)}</select></td><td>{number(p.quantity)}</td><td className="market-price">{p.marketPrice?<><strong>{money(p.marketPrice,p.marketPrice<10?3:2)}</strong>{p.priceChangePct!=null&&<small className={p.priceChangePct>=0?"positive":"negative-text"}>{p.priceChangePct>=0?"+":""}{number(p.priceChangePct,2)}%</small>}</>:<span>—</span>}</td><td><strong>{money(p.value)}</strong></td><td>{money(p.cost)}</td><td className={profit>=0?"positive":"negative-text"}><strong>{profit>=0?"+":""}{money(profit)}</strong><small>{p.cost?`${profit>=0?"+":""}${number(profit/p.cost*100,1)}%`:"—"}</small></td><td><div className="weight"><span style={{width:`${totalValue?p.value/totalValue*100:0}%`}}/></div><small>{totalValue?number(p.value/totalValue*100,1):"0,0"}%</small></td></tr>})}</tbody></table></div>:<EmptyPositions onImport={()=>fileRef.current?.click()} onDemo={()=>setData(demoData)}/>}</section>
    <section className={`dropzone ${dragging?"dragging":""}`} onDragOver={e=>{e.preventDefault();setDragging(true)}} onDragLeave={()=>setDragging(false)} onDrop={e=>{e.preventDefault();setDragging(false);importFile(e.dataTransfer.files[0])}} onClick={()=>fileRef.current?.click()}><span className="upload-icon">⇧</span><div><strong>Upuść pełny eksport XTB</strong><p>ZIP, XLSX, XLS lub CSV · obsługa wielu rachunków</p></div><button>Wybierz plik</button></section></>}
    ...
    {addingAsset&&<div className="modal-backdrop" role="presentation" tabIndex={-1} onKeyDown={e=>{if(e.key==="Escape")setAddingAsset(false)}} onMouseDown={e=>{if(e.currentTarget===e.target)setAddingAsset(false)}}>
      <section className="asset-modal" role="dialog" aria-modal="true" aria-labelledby="asset-modal-title">
        <header><div><p className="eyebrow">Poza XTB</p><h2 id="asset-modal-title">Dodaj własne aktywo</h2></div><button className="modal-close" type="button" onClick={()=>setAddingAsset(false)} aria-label="Zamknij">×</button></header>
        <div className="asset-type-tabs" aria-label="Rodzaj aktywa">{(["Krypto","Gotówka","Inne"] as ManualKind[]).map(kind=><button type="button" key={kind} className={manualKind===kind?"active":""} onClick={()=>setManualKind(kind)}>{kind}</button>)}</div>
        <form key={manualKind} onSubmit={addManualAsset}><div className="form-grid">
          {manualKind==="Krypto"&&<><label className="field"><span>Symbol</span><input name="symbol" placeholder="BTC" required autoFocus /></label><label className="field"><span>Nazwa</span><input name="name" placeholder="Bitcoin" /></label><label className="field"><span>Ilość</span><input name="quantity" type="number" min="0" step="any" placeholder="0,25" required /></label><label className="field"><span>Średnia cena zakupu (PLN)</span><input name="averagePrice" type="number" min="0" step="any" placeholder="180000" /></label><label className="field full-field"><span>CoinGecko ID (tylko dla mniej znanych monet)</span><input name="priceId" placeholder="np. render-token" /></label></>}
          {manualKind==="Gotówka"&&<><label className="field"><span>Waluta</span><select name="currency" defaultValue="PLN" autoFocus><option>PLN</option><option>EUR</option><option>USD</option><option>GBP</option><option>CHF</option><option>DKK</option><option>SEK</option><option>NOK</option></select></label><label className="field"><span>Kwota</span><input name="quantity" type="number" min="0" step="any" placeholder="10000" required /></label><label className="field"><span>Kurs zakupu do PLN</span><input name="fx" type="number" min="0" step="any" defaultValue="1" required /></label></>}
        ...
        </div><p className="form-hint">Krypto i waluty dostają bieżącą cenę automatycznie. Wartość całego portfela jest liczona w PLN.</p><div className="modal-actions"><button type="button" className="secondary-action" onClick={()=>setAddingAsset(false)}>Anuluj</button><button type="submit" className="primary-action">Dodaj do portfela</button></div></form>
    ...
    <footer><span>Kapitał</span><p>To narzędzie informacyjne — nie stanowi porady inwestycyjnej. Notowania mogą być opóźnione względem rynku.</p><b>Notowania online</b></footer></section>
  </main>
}
