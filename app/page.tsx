...
import { calculateTaxSummary, isRetirementAccount, taxYears } from "../lib/tax-calculator";
...
export default function Home({initialView="pulpit"}:{initialView?:AppView}={}){
  ...
  const [taxPriorLoss,setTaxPriorLoss]=useState(0);
  ...
  const taxYearOptions=useMemo(()=>taxYears(data.trades,data.cash,today.getFullYear()),[data.cash,data.trades]);
  const taxSummary=useMemo(()=>calculateTaxSummary({year:taxYear,trades:data.trades,cash:data.cash,eligiblePriorLoss:taxPriorLoss,extraCosts:taxExtraCosts}),[data.cash,data.trades,taxExtraCosts,taxPriorLoss,taxYear]);
  const taxYearTrades=useMemo(()=>data.trades.filter(item=>Number(item.date.slice(0,4))===taxYear&&!isRetirementAccount(item.account)).sort((a,b)=>+new Date(b.date)-+new Date(a.date)),[data.trades,taxYear]);
  const hasManualCrypto=data.positions.some(item=>item.manual&&item.assetClass==="Krypto");
  ...
  return <main className="app-shell">
    ...
    {view==="podatki"&&<section className="tax-page">
      ...
      <section className="tax-hero-grid">
        <article className="tax-total-card"><span>Szacowany podatek do zapłaty</span><strong>{pln(taxSummary.totalTaxDue,0)}</strong><small>Sprzedaż aktywów + zagraniczne dywidendy</small></article>
      ...
      <section className="tax-grid">
        <article className="panel tax-card"><div className="tax-card-head"><div><p className="eyebrow">PIT-38 · sprzedaż</p><h3>Papiery wartościowe</h3></div><span className="tax-rate">19%</span></div><div className="tax-rows"><div><span>Przychód ze sprzedaży</span><b>{pln(taxSummary.trades.revenue)}</b></div><div><span>Koszty uzyskania</span><b>{pln(taxSummary.trades.costs)}</b></div><div><span>Dochód / strata</span><b className={taxSummary.trades.result<0?"negative-text":""}>{pln(taxSummary.trades.result)}</b></div><div><span>Odliczona strata z lat ubiegłych</span><b>− {pln(taxSummary.trades.eligiblePriorLoss)}</b></div><div className="tax-result"><span>Podstawa opodatkowania</span><b>{pln(taxSummary.trades.taxableBase,0)}</b></div><div className="tax-result"><span>Podatek</span><b>{pln(taxSummary.trades.tax,0)}</b></div></div><div className="tax-inputs"><label><span>Strata z poprzednich lat możliwa do odliczenia</span><input type="number" min="0" step="0.01" value={taxPriorLoss||""} placeholder="0,00" onChange={event=>setTaxPriorLoss(Math.max(0,Number(event.target.value)||0))}/></label><label><span>Inne udokumentowane koszty maklerskie</span><input type="number" min="0" step="0.01" value={taxExtraCosts||""} placeholder="0,00" onChange={event=>setTaxExtraCosts(Math.max(0,Number(event.target.value)||0))}/></label></div></article>
        ...
        <article className="panel tax-card tax-control-card"><div className="tax-card-head"><div><p className="eyebrow">Kontrola danych</p><h3>Wiarygodność wyliczenia</h3></div><Info size={20}/></div><ul><li className={taxSummary.trades.incompleteValues?"warning":"ok"}><span/>{taxSummary.trades.incompleteValues?`${taxSummary.trades.incompleteValues} transakcji nie ma pełnej wartości zakupu i sprzedaży — użyliśmy wyniku netto.`:"Każda transakcja ma wartość zakupu i sprzedaży."}</li><li className={hasManualCrypto?"warning":"ok"}><span/>{hasManualCrypto?"Ręcznie dodane krypto nie ma historii transakcji i nie jest jeszcze ujęte w podatku.":"Brak ręcznego krypto wymagającego historii sprzedaży."}</li><li className={taxYear===today.getFullYear()?"warning":"ok"}><span/>{taxYear===today.getFullYear()?"To rok w trakcie — wynik zmieni się po kolejnych operacjach.":"Wybrany rok jest zakończony."}</li></ul></article>
      </section>
      <section className="panel tax-transactions"><div className="panel-head"><div><p className="eyebrow">Źródło wyliczenia</p><h3>Transakcje uwzględnione za {taxYear}</h3></div><span className="count-pill">{taxYearTrades.length}</span></div>{taxYearTrades.length?<div className="table-wrap"><table><thead><tr><th>Data</th><th>Instrument</th><th>Rachunek</th><th>Przychód</th><th>Koszt</th><th>Wynik</th></tr></thead><tbody>{taxYearTrades.map(item=><tr key={item.id}><td>{dateLabel(item.date)}</td><td><strong>{item.symbol}</strong></td><td><span className="account-pill">{item.account||"PLN"}</span></td><td>{item.saleValue==null?"—":pln(item.saleValue)}</td><td>{item.purchaseValue==null?"—":pln(item.purchaseValue)}</td><td className={item.result<0?"negative-text":"positive"}><strong>{pln(item.result)}</strong></td></tr>)}</tbody></table></div>:<p className="empty-inline">Brak zamkniętych transakcji na zwykłym rachunku w tym roku.</p>}</section>
      <div className="tax-disclaimer"><ReceiptText size={20}/><div><strong>To robocze wyliczenie, nie gotowa deklaracja</strong><p>Sprawdź kwoty z PIT-8C, kursy NBP, koszty spoza raportu i limity odliczenia strat. Kalkulator nie wysyła deklaracji.</p><div><a href="https://www.podatki.gov.pl/podatki-osobiste/pit/informacje-podstawowe/co-jest-opodatkowane/zbycie-akcji" target="_blank" rel="noreferrer">Zasady sprzedaży akcji</a><a href="https://www.podatki.gov.pl/media/11079/broszura-do-pit-38-za-2025-r.pdf" target="_blank" rel="noreferrer">Broszura PIT-38 MF</a></div></div></div>
    </section>}
