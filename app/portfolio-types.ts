...
export type PriceStatus = { loading:boolean; updatedAt:string|null; updated:number; missing:string[] };
...
export type PerformancePoint = { month:string; label:string; capitalGain:number; portfolioPct:number; benchmarkPct:number; investedCapital:number };
export type PerformanceResponse = { points:PerformancePoint[]; benchmark:{symbol:string;name:string}; missing:string[]; methodology:string };
