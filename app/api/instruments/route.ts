...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (kind === "crypto") {
      const response = await fetch(`https://api.coingecko.com/api/v3/search?query=${encodeURIComponent(query)}`, { headers: { "accept": "application/json", "user-agent": "LekkiPortfel/1.0" } });
