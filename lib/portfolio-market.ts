import { isStablecoin } from "./asset-class.ts";

export const ALL_MARKETS="all" as const;

export type MarketFilter=typeof ALL_MARKETS|"gpw"|"foreign"|"crypto"|"stable"|"cash"|"other";
...
export const marketFilterOrder:MarketFilter[]=[ALL_MARKETS,"gpw","foreign","crypto","stable","cash","other"];
...
export function marketFor(item:MarketItem):Exclude<MarketFilter,"all">{
  ...
  if(assetClass==="STABLE"||(isStablecoin(item.symbol)&&(assetClass.includes("KRYPTO")||assetClass.includes("CRYPTO"))))return"stable";
